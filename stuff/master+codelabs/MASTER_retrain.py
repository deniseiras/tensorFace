...
def run_final_eval(sess, model_info, class_count, image_lists, jpeg_data_tensor,
  ...
  Args:
    sess: Session for the face_trainer graph.
...
def build_eval_session(model_info, class_count):
  """Builds an restored eval session without face_trainer operations for exporting.
...
def main(_):
  ...
  with tf.Session(graph=graph) as sess:
    ...
    # Create a face_trainer saver that is used to restore values into an eval graph
    ...
    for i in range(FLAGS.how_many_training_steps):
      ...
      if (intermediate_frequency > 0 and (i % intermediate_frequency == 0)
          and i > 0):
        # If we want to do an intermediate save, save a checkpoint of the face_trainer
    ...
    # After training is complete, force one last save of the face_trainer checkpoint.
    train_saver.save(sess, CHECKPOINT_NAME)
