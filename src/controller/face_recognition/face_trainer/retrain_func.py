...
# we're using for Inception v3. These include things like tensorFace names and their
...
def get_image_path(image_lists, label_name, index, image_dir, category):
    ...
    if not category_list:
        str_err = 'Label {} has no images in the category {}.'.format(label_name, category)
        tf.logging.fatal(str_err)
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor,
    ...
    with tf.name_scope('face_trainer'):
...
def prepare_file_system():
    ...
    # TODO - Denis - not erasing in trainer.py
    try:
        if tf.gfile.Exists(FLAGS.summaries_dir):
            tf.gfile.DeleteRecursively(FLAGS.summaries_dir)
        tf.gfile.MakeDirs(FLAGS.summaries_dir)
    except:
        debug.msg("Could not erase {}".format(FLAGS.summaries_dir))
...
def mainRetrain(flags_par, _):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        train_writer = tf.summary.FileWriter(FLAGS.summaries_dir + '/face_trainer',
                                             sess.graph)
        ...
        debug.msg('Final test accuracy = %.4f%% (N=%d)\n' %
                  (test_accuracy * 100, len(test_bottlenecks)))
...
def parseArguments():
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help='How many images to face_trainer on at a time.'
    )
    parser.add_argument(
        '--test_batch_size',
        type=int,
...
#   tf.app.run(main=mainRetrain, argv=[sys.argv[0]] + unparsed)
